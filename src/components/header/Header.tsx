        ...
        <div className={`py-2   items-stretch header ${isActive ? "bg-[#0e2f46] active" : ""}`}>
          <div className={`${Classes["container"]} `}>
            <div className="flex flex-row justify-between">
              <Link to="/" className="logo_div flex items-center  lg:gap-1 xl:gap-3 cursor-pointer">
                <div>
                  <img src={bottomHeaderData[0].logo} alt="" className=" h-[35px] lg:h-[57px] max-w-[100%] " />
                </div>
                <div>
                  <h2 className={` font-bold lg:text-[1rem] xl:text-[1.4rem] ${isActive ? "text-white" : location.pathname !== "/" ? "text-white" : ""} `}>
                    SHRI RAMSWAROOP
                        ...
                        <>
                          {
                            ...
                            >
                              <span className={`flex items-center ${isActive ? "text-white" : location.pathname !== "/" ? "text-white" : ""
                                } hover:text-[#ffb703]`}>{data.item}</span>
                            ...
                            >
                              ...
                              {data?.subItem && (
                                <div className={`absolute top-[59px] xl:top-[60px] right-0 bg-white rounded-md  manage-shadow-drop-down`}>
                                  ...
                                  <ul
                                    className={`transition-all w-full min-w-[250px] duration-300   h-full overflow-hidden max-h-0 group-hover:max-h-[500px] group-hover:border rounded-md border-[#ffb703]  group-hover:overflow-y-auto hide-scrollbar`}
                                  >
            ...
            {bottomHeaderData[0].navitems.map((data, index) => {
              return (
                <li key={index} className={`relative w-full  ${data.item === "Login" && "hidden"} text-sm  uppercase cursor-pointer py-2 ${data.id !== 0 && "border-t"}  `}>
                  <Link to={data.path} key={index} className={`  ${location.pathname === data.path ? "active" : ""} w-full block   [&.active]:text-[#ffb703] `}>
